...
(defun game-loop ()
  (unless (or (player-dead) (monsters-dead))
    ...
    (dotimes (k (1+ (truncate (/ (max 0 *player-agility*) 15))))
      ;; Is this check even needed because nothing would kill a monster
      ;; between now and the unless test
      (unless (monsters-dead)
        (show-monsters)
        (player-attack)))
    (fresh-line)
    ;; Is there a more declarative way to 
    ;; a) filter out the dead monsters and 
    ;; b) perform an operation on elements in a list
    (map 'list
         (lambda(m)
...
; should be player-show to mirror player-attack and monster-show
(defun show-player ()
  (fresh-line)
...
(defun player-attack ()
  ...
  (case (read)
    ...
    (d (let ((x (randval (truncate (/ *player-strength* 6)))))
         ...
         (unless (monsters-dead)
           (monster-hit (pick-monster) x))))
    ; Kinda should handle non-r characters seperatly
